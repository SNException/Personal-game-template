...
IF DEFINED VERBOSE (
    ...
    FOR /R %%f IN (*.java) DO (
        ECHO    ^> %%f
        FOR /F %%a IN ('TYPE "%%f"^|FIND /C /V  "" ') DO SET /A total+=%%a&ECHO %%f %%a >nul 2>&1
        SET /A filecount+=1
    )
    POPD
...
IF EXIST %RES_DIR% (
    ...
    COPY %RES_DIR%\*.* %BUILD_DIR%\%RES_DIR% >nul 2>&1
)
...
IF "%LIBS%"=="" (
    ...
    javadoc -d %DOC_DIR% -cp %LIBS% %SOURCES%
)
